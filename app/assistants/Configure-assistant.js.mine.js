...
ConfigureAssistant.prototype.setup = function() {
	...
	this.controller.setupWidget("email",
         { hintText: $L('Linkedin ID ..'), textReplacement : false},
         this.emailModel = {
             disabled: false
    });
     //value: "helama28",
...
ConfigureAssistant.prototype.submitReceivedHandler = function(event) {
	...
	Mojo.Log.info('Submit received - email:', $('email').mojo.getValue(), 'password:', $('pass-field').mojo.getValue());
...
ConfigureAssistant.prototype.storeInDepot = function () {
	...
	if (this.names >= 25 && this.work >= 25 && this.photos >= 5) {
		...
		this.controller.modelChanged(this.progressbarModel);
		$("temp").innerHTML = "You don't have enough contacts to play the game! Get more linked in, and then come back and reimport your contacts."; 
	}
...
ConfigureAssistant.prototype.contactsLoaded = function () {
	this.progressbarModel.title = "Contacts successfully downloaded";
	this.controller.modelChanged(this.progressbarModel);
...
ConfigureAssistant.prototype.updateProgress = function () {
	this.progressbarModel.title = "Downloading " + Math.round((this.contactsReceived / this.totalContacts) * 100) + "% complete";
...
ConfigureAssistant.prototype.failureHandler = function(code, event) {
	Mojo.Log.info("-----ConfigureAssistant.prototype.failureHandler");
	if (code == 1) {
